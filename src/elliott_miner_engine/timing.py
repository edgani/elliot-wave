...
from .fib import closeness_to_set
...
def _make_projection(
    ...
    fit = closeness_to_set(actual, projected_lengths, tol=max(2.0, 0.35 * max(projected_lengths or [actual]))) if projected_lengths else None
    return WaveDurationProjection(
        ...
        projected_end_index_central=None if central is None else start_pivot.idx + central,
        projected_end_index_low=None if low is None else start_pivot.idx + low,
        projected_end_index_high=None if high is None else start_pivot.idx + high,
        fit_score=fit,
        basis=basis,
        status=status,
    )
...
def impulse_wave_duration_projections(pivots: List[Pivot]) -> List[WaveDurationProjection]:
    if len(pivots) != 6:
        return []
    ...
    out = [
        ...
        _make_projection(
            ...
            pivots[1],
            pivots[2],
            _projected_lengths(d1, TIME_SET_CORRECTIVE),
            [0.6, 1.0, 0.8, 0.5],
            'Projected from W1 duration using Miner-style corrective time ratios 0.382/0.618/1.0/1.618 of W1.',
        ),
        _make_projection(
            'W3',
            pivots[2],
            pivots[3],
            _projected_lengths(d1, TIME_SET_EXTENDED),
            [0.5, 0.9, 1.0, 0.6],
            'Projected from W1 duration using motive-extension time ratios 0.618/1.0/1.618/2.618 of W1.',
        ),
    ]
    ...
    out.append(
        _make_projection(
            ...
            w4_candidates,
            w4_weights,
            'Projected from W2 and W1 durations. Corrective Wave 4 often clusters around 0.618/1.0/1.618 of W2 and 0.382/0.618/1.0 of W1.',
        )
    )
    ...
    out.append(
        _make_projection(
            ...
            w5_candidates,
            w5_weights,
            'Projected from W1 and W3 durations. Wave 5 often clusters around 0.618/1.0/1.618 of W1 and 0.382/0.618/1.0 of W3.',
        )
    )
    return out


def zigzag_wave_duration_projections(pivots: List[Pivot]) -> List[WaveDurationProjection]:
    if len(pivots) != 4:
        return []
    ...
    out = [
        ...
        _make_projection(
            ...
            pivots[1],
            pivots[2],
            _projected_lengths(d_a, TIME_SET_CORRECTIVE),
            [0.6, 1.0, 0.8, 0.5],
            'Projected from A duration using corrective time ratios 0.382/0.618/1.0/1.618 of A.',
        ),
        _make_projection(
            'C',
            pivots[2],
            pivots[3],
            _projected_lengths(d_a, TIME_SET_EXTENDED),
            [0.6, 0.9, 1.0, 0.7],
            'Projected from A duration using motive-like time ratios 0.618/1.0/1.618/2.618 of A.',
        ),
    ]
    return out


def triangle_wave_duration_projections(pivots: List[Pivot]) -> List[WaveDurationProjection]:
    if len(pivots) != 6:
        return []
    names = ['A', 'B', 'C', 'D', 'E']
